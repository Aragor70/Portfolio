    ...
    return (
        <Fragment>
            ...
            */}
            ...
            {
                Boolean(project?.images?.length) && <div className="main-image" >{
            ...
            <article className="single-content gap-single">
                ...
                <div ref={fadeInUpElement} className="no-opacity">
                    ...
                    <ul className="icons">
                        {
                            Boolean(project.icons?.length) && project?.icons?.map((element: SingleImage) => element.isFile ? <li key={element.id} className="icon"><img src={`/assets/icons/${element.name}`} alt={element.label} /><span>{element.label}</span></li> : <li className="icon"  key={element.id}><i className={element.name}></i><span>{element.label}</span></li>)
                        }
                    </ul>
                    ...
                    <ul className="more-about">
                        ...
                        {
                            Boolean(project.repositories?.length) && <li className="icon-box" onClick={() => window.open(project?.repositories[0]?.url, "_blank")}><i className="fas fa-code fa"></i></li>
                        }
                        {
                            Boolean(project.website) && <li className="icon-box" onClick={() => window.open(project.website, "_blank")}><i className="fab fa-chrome fa"></i></li>
                        }

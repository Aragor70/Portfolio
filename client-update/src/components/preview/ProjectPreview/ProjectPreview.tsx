        ...
        <Fragment>
            <article className={ style?.className || ''}>
                ...
                <div className={styles.sectionImage}>
                    {
                        Boolean(props.images?.length) && <img src={props?.images[0]?.name} alt={props.images[0]?.label} onError={(e: any) => {e.target.style.display = 'none'}} />
                    }
                ...
                <div className={clsx(styles.param, styles.noOpacity)} ref={fadeInUpElement}>
                    ...
                    <ul className={styles.icons}>
                        {
                            Boolean(props.icons?.length) && props?.icons?.map((element: any) => element.isFile ? <li key={element.id} className="icon"><img src={`/assets/icons/${element.name}`} alt={element.label} /><span>{element.label}</span></li> : <li className="icon"  key={element.id}><i className={element.name}></i><span>{element.label}</span></li>)
                        }
                    </ul>
                    
                    <ul className={styles.moreAbout}>
                        {
                            Boolean(props.name) && <Link to={`/software_projects/${props.name}`}><Translate tKey='sd.project.link.show.details' /></Link>
                        }
                        {
                            Boolean(props.repositories?.length) && <li className={styles.iconBox} onClick={() => window.open(props?.repositories[0]?.url, "_blank")}><i className="fas fa-code fa"></i></li>
                        }
                        {
                            openEdit['website'] ? <textarea className="textarea" value={formData?.website || ''} name="website" onChange={ (e: any) => handleChange(e)} contentEditable suppressContentEditableWarning placeholder="What is the website url?"></textarea> : Boolean(props.website) && <li className={styles.iconBox} onClick={() => window.open(props.website, "_blank")}><i className="fab fa-chrome fa"></i></li>
                        }
